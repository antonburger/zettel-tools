...
export default async function pasteAttachment(editor: vscode.TextEditor) {
    ...
    if (!workspaceFolder) {
        await vscode.window.showErrorMessage(
            "Target document must belong to an open workspace folder."
        );
        return;
    }
    ...
    const copyQOutput = await runCopyQ("copyq", getCopyQScript(mimeTypes)).then(
        undefined,
        elaborateError("There was an error running copyq")
    );
    ...
    const [sourceMimeType, sourceFilename] = parseCopyQLine(
        lines[0],
        mimeTypes
    );
    ...
    const attachmentDirectory = workspaceFolder.uri.with({
        path: workspaceFolder.uri.path + "/attachments",
    });
    await vscode.workspace.fs
        .createDirectory(attachmentDirectory)
        .then(
            undefined,
            elaborateError("Couldn't create attachment directory")
        );

    // Construct a name for the attachment file and copy it into the workspace
    const targetBaseName = createBaseName();
    const targetExtension = sourceMimeType
        ? mimeExtensions.get(sourceMimeType)!
        : path.extname(sourceFilename);
    const targetUri = attachmentDirectory.with({
        path: `${attachmentDirectory.path}/${targetBaseName}${targetExtension}`,
    });
    await vscode.workspace.fs
        .copy(sourceUri, targetUri, { overwrite: true })
        .then(undefined, elaborateError("Couldn't copy attachment"));

    // Build and insert a link to the attachment for the current editor
    const relativeTargetPath = path
        .relative(path.dirname(editor.document.uri.fsPath), targetUri.fsPath)
        .replace(/\\/g, "/");
    ...
    return new Promise((resolve, reject) => {
        ...
        ps.on("exit", (code) => {
            if (code === 0) {
                ...
                reject(
                    new Error(
                        `copyq invocation returned ${code}: ${stderr.join(
                            "\n"
                        )}`
                    )
                );
...
function createBaseName() {
    const date = new Date();
    return `${date.getFullYear()}-${pad2Digits(
        date.getMonth() + 1
    )}-${pad2Digits(date.getDate())}_${pad2Digits(
        date.getHours()
    )}-${pad2Digits(date.getMinutes())}-${pad2Digits(date.getSeconds())}`;
